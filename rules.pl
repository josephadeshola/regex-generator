...
% Recognize characters as themselves, either escaped or enescaped as necessary.
regex_tokens([C|Cs], Acc, Rs, F) :-
    unescaped(C), recurse(Cs, [class(C)|Acc], Rs, F)
  ; escaped(C, E), recurse(Cs, [class(E)|Acc], Rs, F).

% Recognize character classes.
regex_tokens([C|Cs], Acc, Rs, F) :-
    digit(C), recurse(Cs, [class('\\d')|Acc], Rs, F)
  ; upper(C),  recurse(Cs, [class('[A-Z]')|Acc], Rs, F)
  ; lower(C), recurse(Cs, [class('[a-z]')|Acc], Rs, F)
  ; letter(C), recurse(Cs, [class('[a-zA-Z]')|Acc], Rs, F)
  ; word(C), recurse(Cs, [class('\\w')|Acc], Rs, F).
...
split_respecting_arity(R, [O], Os, [O|Os]) :- single_char(R), !.
split_respecting_arity(_, M, N, O) :- append(M, N, O).
